...
use futures::{StreamExt, SinkExt, channel::mpsc::UnboundedSender};
use web_sys::SubmitEvent;
use crate::string;
use crate::{structs::*, frontend::{structs::State, table::Table}};
...
pub fn App() -> View {
    ...
    let ws_sender: Signal<Option<UnboundedSender<Message>>> = create_signal(None::<UnboundedSender<Message>>);
    let user_name = create_signal(String::new());
    let master_is_there = create_selector(move || users.get_clone().into_iter().any(|user|user.role() == Role::Master));
    ...
    spawn_local({
        ...
        async move {
            ...
            let ws = WebSocket::open(&HOST)
                .expect("no se pudo conectar al websocket");
                ...
                async move {
                    while let Some(msg) = read.next().await {
                        if let Ok(Message::Text(txt)) = msg {
                            let current_users = serde_json::from_str::<Vec<User>>(&txt).unwrap();
            ...
            State::NotLogged => view!{
                form(on:submit=move |ev:SubmitEvent|{
                    ...
                    spawn_local(async move {
                        ...
                        let user = User::new(user_role.get_clone().into(),user_name.get_clone().as_str(), None);
                        state.set(State::Logged(user.clone()));
                        send.get_clone().unwrap().send(Message::Text(serde_json::to_string(&MessageText{ room: String::new(), message_type: EventType::Start, user }).unwrap())).await.unwrap();
                    });
                    ...
                    input(r#type="text", placeholder="Input text", bind:value=user_name){}
